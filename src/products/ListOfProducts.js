...
import { productdeleteAction, productdeleteDataOfReset, productsAction } from "../redux/actions/productsActions";
import ProductImg from "../imagesApp/product.png"
...
const ListOfProducts = () => {
    ...
    useEffect(() => {
        if (productsData?.data) {
            setProductsDetails(productsData?.data);
        }
        if (productsData?.error) {
            setError(productsData?.error?.msg);
        }
    }, [productsData]);
    ...
    const { deleteproductData } = useSelector((state) => state.products);

    const confirmDelete = () => {
        dispatch(productdeleteAction(deleteid));
    ...
    useEffect(() => {
        if (deleteproductData && deleteproductData?.data) {
            toast.success(deleteproductData?.data?.msg);
            dispatch(productdeleteDataOfReset())
            dispatch(productsAction());
        }
        if (deleteproductData && deleteproductData?.error) {
            toast.error(deleteproductData?.error?.msg);
            dispatch(productdeleteDataOfReset())
        }
            ...
            <div className="px-10">
                ...
                <div className="flex justify-between items-center mt-6">
                    <div className="flex justify-start items-center gap-3 ">
                        <div>
                            <img src={ProductImg} alt="category" />
                        </div>
                        <div>
                    ...
                    <div>
                        <button
                            ...
                            type="button" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Add Product</button>
                    </div>
                </div>
                ...
                <div className="mt-8 relative overflow-x-auto ">
                    <table className="w-full   text-sm text-center rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            ...
                            {
                                productsDetails?.data?.length > 0 ?

                                    (
                                        productsDetails?.data?.map((items, index) => {
                                            return (
                                                <>
